...
htmlFiles.forEach(filePath => {
    ...
    lines.forEach((line, idx) => {
        if (line.toLowerCase().includes('sparkle') || line.includes('✨') || line.toLowerCase().includes('lucide-sparkle') || line.toLowerCase().includes('lucide-sparkles')) {
            findings.push({ file: relativePath, line: idx + 1, content: line.trim() });
        }
    });
});
