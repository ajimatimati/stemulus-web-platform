...
broken_str = """  <div class="footer-bottom">
...
restored_str = """  <div class="footer-bottom">
...
restored_str = restored_str.replace("\r\n", "\n")

if broken_str in content:
    content = content.replace(broken_str, restored_str)
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("Success! Restored programs.html scripts.")
else:
    # try slightly different whitespace matching
    broken_str_2 = broken_str.replace("  ", " ")
    content_norm = content.replace("  ", " ")
    if broken_str_2 in content_norm:
        # replace using standard python split/re
        print("Found with normalized spaces, trying regex/manual patch...")
    else:
        print("Could not find the exact broken string in programs.html")
